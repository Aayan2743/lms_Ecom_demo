    ...
    setTimeout(() => {
      ...
      if (isPhone) {
        ...
        if (password.length >= 6) {
          ...
          if (isValidRegisteredUser || adminMatch) {
            const userData = adminMatch
              ? { email: adminMatch.email, name: "Admin", role: "admin" }
              : { ...savedUser, role: savedUser.role || "customer" };
            login(userData); // ✅ Update global state
            
            setSuccessMsg("Login Successful! Redirecting...");
            navigate(adminMatch ? "/admin" : "/profile", { replace: true });
          } else {
